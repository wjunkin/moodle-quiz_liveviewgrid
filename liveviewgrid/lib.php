...
/**
 * Standard plugin entry points of the quiz liveviewgrid report.
 *
 ...
 */
function quiz_liveviewgrid_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('quiz/liveviewgrid:view', $context);
...
/**
 * Quiz liveviewgrid report cron code. Deletes cached data more than a certain age.
 */
function quiz_liveviewgrid_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quiz liveviewgrid cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('quiz_liveviewgrid', 'timemodified < ?', array($expiretime));
