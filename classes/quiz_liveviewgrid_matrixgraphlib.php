...
/**
 * Quiz liveviewgrid quiz_liveviewgrid_matrixgraphlib class.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 **/
class quiz_liveviewgrid_matrixgraphlib {
    /**
    ...
    public function draw_set($set, $offset) {
        $correct = optional_param('correct', 0, PARAM_INT);// If correct=1 there is at least one correct row answer.
        if ($correct == 1) {
            $ccolor = 'green';
            $xcolor = 'red';
        } else {
        ...
        while ($yvalue == 0) {
            ...
            $nonzeroi ++;
            if ($nonzeroi > count($this->y_data['responses'])) {
                $yvalue = -1;
            }
        }
        $this->ymultiplier = $this->calculated['y_plot']['responses'][$nonzeroi - 1] / $this->y_data['responses'][$nonzeroi - 1];
        foreach ($this->x_data as $index => $x) {
            ...
            if (($bar != 'none') && (string)$thisy != 'none') {
                ...
                $newy = $this->y_datax['responses'][$index] * $this->ymultiplier + $thisy; $newcolor = 'green';
                $this->bar($thisx, $newy, $bar, $barsize, $xcolor, $offset, $set, $yoffset);
                $this->bar($thisx, $thisy, $bar, $barsize, $ccolor, $offset, $set, $yoffset);   // Moodle.
            }
    ...
    private function prepare_label_text($label) {
        if (right_to_left() && !preg_match('/[0-9]/i', $label)) {
